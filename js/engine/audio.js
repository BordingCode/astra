...
export const sfx = {
  ...
  // --- ASTRA physics events ---
  launch()  { swell({ dur: 0.5, gain: 0.10, cutoff: 1100 });
              tone(noteFreq(2, 0), { dur: 0.35, gain: 0.12, type: 'triangle' }); },
  land()    { tone(noteFreq(0, -1), { dur: 0.45, gain: 0.16, type: 'sine' });
              swell({ dur: 0.3, gain: 0.08, cutoff: 420 }); },
  tick()    { tone(noteFreq(4, 1), { dur: 0.10, gain: 0.045, type: 'sine' }); },
  win()     { const seq = [0, 4, 7, 9]; seq.forEach((d, k) =>
                setTimeout(() => tone(noteFreq(d, 1), { dur: 0.8, gain: 0.13, type: 'triangle' }), k * 110));
              swell({ dur: 1.3, gain: 0.08, cutoff: 1500 }); },
};
